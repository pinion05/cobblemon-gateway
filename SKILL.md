...
description: "새 플레이어 PC를 코블몬 서버에 자동 연결. Tailscale + Fabric + Cobblemon 설치."
---
...
이 스킬은 새 플레이어의 PC에서 실행된다. 목표: 서버 접속에 필요한 모든 클라이언트 세팅을 자동화한다. 사용자는 공유 링크와 서버 주소를 입력하는 것만 하면 된다.

> **호스트**가 서버를 이미 구축해두었다. 서버 구축 세부 사항은 이 스킬의 범위 밖이다. `HOST.md` 참고.
...
| W1-B | S4 | Fabric Loader 설치됨 | 런처 프로필에 `fabric-loader-*1.21.1` 존재 | Java 21, Minecraft 경로 |
| W1-B | S5 | Cobblemon mod 설치됨 | `mods/`에 Cobblemon jar 존재 | Minecraft 경로 |
| W1-B | S6 | Fabric API 설치됨 | `mods/`에 Fabric API jar 존재 | Minecraft 경로 |
| W2 | S2 | Tailscale 로그인 + 공유 수락 | `BackendState=Running`, 공유 노드 peer 표시 | S1, 공유 링크 |
| W3 | S3 | 서버 접근 가능 | `tailscale ping` + TCP 25565 연결 성공 | S2, 서버 주소 |
| W4 | S7 | 실행 안내 완료 | 서버 주소 전달 | S3, S4, S5, S6 |
...
- macOS/Linux: `nohup` 또는 에이전트 도구의 지속 실행 세션 사용.
- Windows: `Start-Process -PassThru` 사용.
...
Tailscale ping과 TCP 25565 연결이 모두 성공하면 S3 PASS다. 다른 포트를 능동적으로 스캔하지 않는다. 실패하면 `tailscale status --json`에서 공유 노드와 서버 온라인 상태를 확인한다.
...
| 공유 노드가 안 보임 | 공유 링크 수락 여부와 서버 온라인 상태 확인 |
| 다른 서버 장비가 보임 | tailnet 초대일 수 있음. 호스트에게 공유 링크 재발급 요청 |
...
- **비밀 보호:** 비밀번호, 인증 정보를 출력하거나 로그에 저장하지 않음
